...
function Comment({ commentData, userId, index }: any) {
    const { commented_by, comment, commented_name, attachment, url, status, comment_id, created_dt } = commentData
    ...
    const onSubmitSelectedFile = (e: any) => {
        dispatch(selectedReviewFile({ status: "1" }, localUserData.leadId, e.target.value))
    }
        ...
        (attachment) && (
            <div tabIndex={1} className='comments__body--message__attachment'>
                <div className='comments__body--message__attachment--item'>
                    <AiOutlineFile />
                    <p id='tax-file'>File : <a href={attachment} target="_blank" rel="noopener noreferrer">{attachment.split('/').pop().replace(/%20/g, '') || 'Tax file'}</a></p>
                </div>
                 {
