...
const Signature = ({ show, toggleSignature, type }: any) => {
  ...
  const submitSignature = async () => {
    try {
      ...
      await dispatch(initSignature(formData, localUserData.leadId))
      toggleSignature()
    } catch (error) {
  ...
  return (
    <Modal title={`${type === 1 ? 'signature' : 'Spouse signature'}`} visible={show} onCancel={toggleSignature} footer={() => (
      <div style={{display: 'flex'}}>
        <Button onClick={toggleSignature}>Cancel</Button>
        <Button onClick={submitSignature} type='primary'>Save Signature</Button>
      </div>
    )}>
        <div style={{border: "1px solid", width: "100%", padding: "22px", marginTop:"5px"}}>
          <SignatureCanvas 
            ref={signatureRef} 
            penColor='black'
