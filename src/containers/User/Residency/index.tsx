...
import { residencyEmptyContextTaxPayer, residencyEmptyContextSpouse, residencyTaxContext, stateOptions } from './constants'
...
const [filesChanged, setFilesChanged] = useState<boolean>(false)
    ...
    const onSubmitResidency = useCallback((values: any) => {
        ...
        console.log('isValidData', !isValidData, residencyArray)
        if(!isValidData){
            dispatch(setError({ status: true, type: 'error', message: 'Form validation Error' }))
            return;
        }
        ...
        //dispatch(updateStateResidency(formData, localStoreData.leadId))
        dispatch(updateStateResidency(formData, leadId))
    },[spouse, taxPayer, taxPayerDetails])
                ...
                <>
                 ...
                 <Row>
                        {leadData?.monthly_rent_paid_states?.length > 1 && (
                            <p> Selected States list: {fileNames(leadData?.monthly_rent_paid_states)}</p>
                        )}
                        </Row> 
                    ...
                    <Row justify={'end'}>
                        ...
                        <Col>
                            <Form.Item>
                                ...
                                <Button type="primary" htmlType="submit" onClick={onSubmitResidency}>
                                    Submit
                                </Button>
                            </Form.Item>
                        </Col>
