...
function FbarFatca() {
  ...
  const onChangeFbarFatcaDetails = useCallback(
    (value: string, name: string, index: number) => {
      formChangedRef.current = true; // ✅ trigger autosave
  ...
  const deleteTaxPayer = useCallback(
    (index: number) => {
  ...
  const onSubmitFbarFatca = (values: any, isAutoSave = false) => {
    setValidate(true);
    if (!isAutoSave) {
    ...
    if (fbarFatcaOptions.fbar === "0" && fbarFatcaOptions.fatca === "0") {
      ...
      isValidData = residencyArray.every((resident: any) => {
        ...
        return isEmptyKeys(copyValues, "ALL", [
          "accountClosedYear",
          "accountOpenedYear",
          "bankMailingAddress",
          "currency",
    ...
    if (residencyArray.length > 0) {
      taxPayer.forEach((taxPayee: any) => {
        ...
        if (copyValues.accountClosedYear) {
          copyValues.accountClosedYear = convertToDateYear(
            copyValues.accountClosedYear
          );
        }
        if (copyValues.accountOpenedYear) {
          copyValues.accountOpenedYear = convertToDateYear(
            copyValues.accountOpenedYear
          );
        }
    ...
    dispatch(
      addFbarFatca(
        { fbarfatcaSettings: finalModifiedData, ...fbarFatcaOptions },
        leadData?.lead_id
      )
    );
    if (!isAutoSave) {
  ...
  useEffect(() => {
    ...
    autoSaveTimerRef.current = setTimeout(() => {
      onSubmitFbarFatca(taxPayer, true);
  ...
  }, [taxPayer, fbarFatcaOptions]); // ✅ add fbarFatcaOptions
  ...
  useEffect(() => {
    if (
      ...
      const { fbarFatcaSettings, first_name, spouseDetails, fatca, fbar } =
        leadData;
      // console.log('fbarfatcaSettings', fbarFatcaSettings)
      if (fbarFatcaSettings?.length > 0 && fbarFatcaSettings) {
        setTaxPayer(
          fbarFatcaSettings.map((fbarFatcaSetting: any) => {
            const {
              account_closed_year,
              account_opened_year,
              address,
            ...
            return {
              accountClosedYear: account_closed_year
                ? dayjs(
                    account_closed_year
                      ? new Date(`${account_closed_year}`).toISOString()
                      : "",
                    "YYYY"
                  )
                : "",
              accountOpenedYear: account_opened_year
                ? dayjs(
                    account_opened_year
                      ? new Date(`${account_opened_year}`).toISOString()
                      : "",
                    "YYYY"
                  )
                : "",
        ...
        <>
          <div>
            ...
            <div style={{ display: "flex", alignItems: "" }}>
              ...
              <Radio.Group
                onChange={(e) => onChangeRadio(e.target.value, "fbar")}
                value={fbarFatcaOptions.fbar}
              >
            ...
            <p>
              The FBAR (Report of Foreign Bank and Financial Accounts) must be
              filed annually by United States persons (Resident) who have a
              financial interest in, or signature authority over, foreign
              financial accounts if the aggregate value of the foreign financial
            ...
            <div style={{ display: "flex", alignItems: "" }}>
              ...
              <Radio.Group
                onChange={(e) => onChangeRadio(e.target.value, "fatca")}
                value={fbarFatcaOptions.fatca}
              >
                <Radio value={"1"}>Yes</Radio>
                <Radio value={"0"}>No</Radio>
              </Radio.Group>
            ...
            <p>
              FATCA (Foreign Account Tax Compliance Act) does not have a
              separate filing requirement like FBAR (Report of Foreign Bank and
              Financial Accounts). Instead, FATCA compliance is typically
              integrated into the regular U.S. tax filing process.
            </p>
            <p>
              - You are unmarried and the total value of your specified foreign
              financial assets is more than $50,000 on the last day of the tax
              year or more than $75,000 at any time during the tax year
            </p>
            <p>
              - You are married filing a joint income tax return and the total
              value of your specified foreign financial assets is more than
              $100,000 on the last day of the tax year or more than $150,000 at
              any time during the tax year.
            </p>
            <hr />
          </div>
          {(fbarFatcaOptions.fbar === "1" ||
            fbarFatcaOptions.fatca === "1") && (
            <div>
