...
function MedicalExpenses() {
  ...
  const [medicalExpensesObj, setMedicalExpensesObj] = useState<any>({
    ...emptyMedicalContext,
  });
  ...
  const formFields: any = useMemo(() => {
    return [
      ...
      // Real Estate Taxes (Text Input)
      {
        ...
        config: {
          rules: [
            { required: false, message: "Please Enter Real Estate Taxes" },
          ],
        },
      },
      ...
      },
      {
        ...
        config: {
          rules: [
            { required: false, message: "Please Enter Contributed To Charity" },
          ],
        },
      },
    ];
  ...
  const onSubmitMedicalIncome = (values: any, isAutoSave = false) => {
    //const copyValues = {...medicalExpensesObj}
    let { realEstateTaxes, stateLocalTaxes, contributedCharity } =
      medicalExpensesObj;
    if (!isAutoSave) {
    ...
    let copyValues = {
      ...medicalExpensesObj,
      realEstateTaxes: realEstateTaxes ? realEstateTaxes : 0,
      stateLocalTaxes: stateLocalTaxes ? stateLocalTaxes : 0,
      contributedCharity: contributedCharity ? contributedCharity : 0,
    };
    // return
    ...
    if (
      typeof copyValues?.form1098 === "string" &&
      copyValues?.form1098.length > 5
    ) {
      delete copyValues.form1098;
    }
            ...
            formFields.filter(
              (field: any) =>
                (field?.parentKey.length > 0 &&
                  ...
                  residentData[field?.parentKey[0]] === "1") ||
                field?.parentKey.length === 0
            )
          );
        }
