...
function GeneralInformation() {
  ...
  const formChangedRef = useRef<boolean>(false); // ✅ Track form changes
  ...
  const onSubmitLead = useCallback(
    (values: any, isAutoSave = false) => {
      ...
      if (dob) copyValues.dob = convertToDate(dob);
      if (dateOfChange) copyValues.dateOfChange = convertToDate(dateOfChange);
      if (primaryPorOfentry)
        copyValues.primaryPorOfentry = convertToDate(primaryPorOfentry);

      // Dependents validation (skip if auto-save)
      if (showDepenedents) {
        const modifiedDependents = dependencies.map((dependent: any) => {
        ...
        const isDependentsValid = modifiedDependents.every((d: any) => {
          if (!isAutoSave) {
            return d.firstName && d.lastName;
          }
          return true; // skip deep validation on auto-save
        });

        if (!isDependentsValid && !isAutoSave) {
          setIsLoading(false);
          return;
        }
      ...
      if (showSpouse) {
        ...
        copyValues.spouseDetails.dob = dayjs(copySpouse.dob).format(
          "YYYY-MM-DD"
        );
      } else {
      ...
      // Dispatch lead creation or update
    ...
    [dependencies, spouseDetails, leadData, showDepenedents, showSpouse]
  );
  ...
  const handleFormChange = (_: any, allValues: any) => {
    ...
    console.log("Field changed. Timer reset for auto-save.");
  };
    ...
    () => [
      {
        ...
        required: true,
        disable: localStorage.getItem("newapplication")
          ? false
          : leadData?.lead_id,
        type: "string",
        config: {
