...
const Dependent = ({
  ...
  useEffect(() => {
    ...
    setfilterFields(
      dependentsFields.filter((field: any) => existingKeys.includes(field.key))
    );
  }, [showFields]);
  ...
  useEffect(() => {
    ...
    if (residentKeys.length > 0) {
      ...
      setfilterFields(
        dependentsFields.filter((field: any) => field?.parentKey.length === 0)
      );
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      >
        <p style={{ margin: 0 }}>
          Dependent {dependents.length > 1 ? index : ""}
        </p>
        <div>
          {dependents.length > 1 && (
            <Button
