...
function DayCareExpense() {
  ...
  const [dayCareData, setDayCareData] = useState<any>([
    { ...dayCareExpensesContext },
  ]);
  ...
  const onChangeDayCareDetails = useCallback(
  ...
  const onSubmitDayCare = (isAutoSave = false) => {
    setValidate(true);
    if (!isAutoSave) {
      setIsLoading(true); // Only for manual save
    }
    let isValidDayCare: boolean = true;
    isValidDayCare = dayCareData.every((dayCare: any) =>
      isEmptyKeys(dayCare, "ALL", [])
    );
    ...
    dispatch(
      addDayCareDetails({ daycareExpenses: modifiedData }, leadData?.lead_id)
    );
    if (!isAutoSave) {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    autoSaveTimerRef.current = setTimeout(() => {
      onSubmitDayCare(true); // ✅ Auto-save call with flag
  ...
  useEffect(() => {
    if (
      ...
      if (dayCareExpenses.length > 0 && dependents === "1") {
        setDayCareData(
          dayCareExpenses.map((dayCare: any) => {
            const {
              provider,
              address,
              ein_ssn,
      ...
      if (dependentDetails.length > 0) {
        setOptions(
          dependentDetails.map((dependent: any) => ({
